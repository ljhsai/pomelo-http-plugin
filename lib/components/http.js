...
var Http = function (app, opts) {
    ...
    this.http.use(express.urlencoded({extended: false}));
...
Http.prototype.loadRoutes = function () {
    this.http.get('/', function (req, res) {
        res.send('pomelo-webserver-plugin ok!');
    });

    var routesPath = path.join(this.app.getBase(), 'app/servers', this.app.getServerType(), 'router');
